...
int optionsCount = WebUI.getText(findTestObject('Object Repository/Page_Dropdown/Page_nopCommerce demo store. Register/MonthEle')).split('\n').size()

def isCamelCase(String str) {
	return str.matches("[a-z][a-zA-Z]*")
}

if (optionsCount > 0) {
for (int i = 1; i <= optionsCount; i++) {
	String optionLocator = 'Object Repository/Page_Dropdown/Page_nopCommerce demo store. Register/MonthEle'

	WebUI.selectOptionByIndex(findTestObject('Object Repository/Page_Dropdown/Page_nopCommerce demo store. Register/MonthEle'), i - 1)
	

	WebUI.delay(2)
	String selectedMonth = WebUI.getText(findTestObject(optionLocator)).trim()
	println("Selected Month: " + selectedMonth)
	
	if (isCamelCase(selectedMonth)) {
		println("Selected Month in Camel Case: " + selectedMonth)
	} else {
		println("Selected Month is not in Camel Case: " + selectedMonth)
	}
}
